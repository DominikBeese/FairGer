...
INPUT_PATH = Path("../../Data/Datasets/Frau.json")
OUTPUT_PATH = Path("../../Analysis/KeywordAnalysis/KeywordDistributionOverTime/DistributionOfFrauKeywordsPerYear.pdf")
...
def load_data(path: Path) -> pd.DataFrame:
    ...
    df = df[["id", "year", "month", "day", "category", "keyword"]].copy()
    df["category"] = "Woman"
    return df
...
def flatten_axes(axes) -> list:
    """Flatten matplotlib axes into a simple list."""
...
def plot_data(dft: pd.DataFrame, order: pd.Index, output_path: Path) -> None:
    ...
    for i, keyword in enumerate(order):
        ...
        ax.set_title(f"${keyword}$", fontsize=10, pad=2)
